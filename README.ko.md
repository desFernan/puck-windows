...
사람에게 묻는다. 펫은 벽이나 화면 가장자리를 타고 천장까지 올라가 거꾸로
매달려 기어다니고, 노치를 켜 두면 그 아래에서 잠깐 멈춰 매달린다. 트레이의
"설정…"에 아바타·이동 속도·노치·테마·자동 시작이 있다. 코드 에디터와
터미널 패널은 Phase 5~6이다.
...
.NET 8 SDK(또는 그 이상 + `net8.0-windows10.0.19041.0` 타게팅) 가 필요하다.
노치가 "이 기계가 무엇을 재생 중인가"를 묻는 곳이 WinRT의
`Windows.Media.Control`이라 버전 있는 TFM이 필요하다. 실제로 요구하는
하한은 Windows 10 1809(10.0.17763)다.
