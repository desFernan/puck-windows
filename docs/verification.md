...
## 자동으로 확인한 것 (2026-08-24)

아래 항목은 스크립트로 실제 앱을 띄워 확인했다. 남은 항목(전체화면 앱 위,
혼합 DPI, 절전/잠금 복귀, 모니터 착탈)은 여전히 손으로 봐야 한다.

- 트레이 아이콘과 메뉴 네 항목, 숨기기/보이기, 종료 후 프로세스 없음
- 오버레이가 투명·최상단이고 그림 위에서만 클릭을 받는다 (여백은 통과)
- 클릭 -> react_click 클립으로 바뀐다
- 드래그하면 발이 커서를 따라오고, 놓으면 떨어져 착지한다
- 세게 던지면 옆벽에서 튕기고 그림이 화면 밖으로 나가지 않는다
- mac에서 만든 아바타 폴더가 그대로 로드되고, 매니페스트를 고친 뒤
  "아바타 다시 불러오기"를 누르면 앱을 끄지 않고 반영된다
- 매니페스트가 깨진 아바타는 건너뛰고 이유가 logs\*.jsonl에 남는다
